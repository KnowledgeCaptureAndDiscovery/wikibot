...
public class Visualization {
	...
	public void changesPastNDaysView(ArrayList<Revision> revisionList, int numOfDays) throws IOException {
		...
		BufferedWriter writer = new BufferedWriter(
				new FileWriter(new File("/home/neha/git/wikibot/visualizations/" + filename)));
	...
	public void display_newsletter(int numOfDays, HashMap<String, String> datasetLinks,
			HashMap<String, String> userLinks, HashMap<String, String> workingGroupLinks,
			HashMap<Article, Integer> subWorkingGroupLinks, HashMap<String, Integer> maxContributorsList) {
		...
		result += "<strong>" + userLinks.size() + " </strong>new authors. <br/>" + " \n";
		...
		for (Entry<String, Integer> entry : maxContributorsList.entrySet()) {
			...
			if (++index == 1) {
				...
				result += "* <strong>" + "<span class='plainlinks'>" + "[" + url + " " + entry.getKey() + "]"
						+ "</span>" + "</strong> " + " has <strong>" + entry.getValue() + "</strong> contributions."
						+ " <br/>";
			}
		}
		result += "<br/>";
		...
		if (userLinks.size() > 0) {
			result += "<strong>Newly added authors:</strong><br/>";
	...
	public void displayFigureText(HashMap<String, Integer> figureMap) {
		...
		text += "Linked Earth contains ";
		text += "<span class='plainlinks'>[http://wiki.linked.earth/Category:Compilation_(L) "
				+ figureMap.get("Compilation") + " Compilations]</span>, ";

		text += "<span class='plainlinks'>[http://wiki.linked.earth/Category:Dataset_(L) " + figureMap.get("Datasets")
				+ " Datasets]</span>, ";

		text += "<span class='plainlinks'>[http://wiki.linked.earth/Category:Observation_(L) "
				+ figureMap.get("Observation") + " Observation]</span> and ";

		text += "<span class='plainlinks'>[http://wiki.linked.earth/Category:Publication_(L) "
				+ figureMap.get("Publication") + " Publications]</span> from ";

		text += "<span class='plainlinks'>[http://wiki.linked.earth/Category:Person_(L) " + figureMap.get("Authors")
				+ " Authors]</span>.<br>";

		text += "The contents of the wiki have been contributed by ";

		text += "<span class='plainlinks'>[http://wiki.linked.earth/Special:ListUsers " + figureMap.get("Contributors")
				+ " Scientists]</span> from ";

		text += "<span class='plainlinks'>[http://wiki.linked.earth/Property:University "
				+ figureMap.get("Institutions") + " Institutions]</span>, organized in ";

		text += "<span class='plainlinks'>[http://wiki.linked.earth/Category:Working_Group "
				+ figureMap.get("Working Groups") + " Working Groups]</span>. ";
