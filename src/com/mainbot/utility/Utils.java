...
public class Utils {
	...
	public static String queryFormulation() {
		...
		// System.out.println("Now printing querystring: " +
	...
	public static boolean login(Bot mainbot) throws JSONException {
		...
		if (checkLogin(mainbot, "user").isEmpty() && checkLogin(mainbot, "bot").isEmpty()) {
			logger.info("User and Bot Assertion Completed.");
		} else {
			if (!checkLogin(mainbot, "user").isEmpty()) {
				logger.error("User Assertion Failed.");
			}
			if (!checkLogin(mainbot, "bot").isEmpty()) {
				logger.error("Bot Assertion Failed.");
			}
		}
	...
	public static String checkLogin(Bot mainbot, String userType) throws JSONException {
		...
		return post.getString("batchcomplete");
	}
